-- Create table for Hybrid Deck (Archetype) compositions
create table if not exists archetype_compositions (
  id uuid default gen_random_uuid() primary key,
  hybrid_archetype_id uuid references archetypes(id) on delete cascade,
  component_archetype_id uuid references archetypes(id) on delete cascade,
  created_at timestamp with time zone default timezone('utc'::text, now()) not null,
  unique(hybrid_archetype_id, component_archetype_id)
);

-- Add hybrid flag to archetypes
alter table archetypes add column is_hybrid boolean default false;

-- RLS for compositions (same as archetypes: view all, manage admin)
